...
#define MODE_GPS        4                 // Mode GPS - wait for GPS sync and use the date as file name
#define MODE_MAX_VALID_MODE MODE_GPS //ROTATE   // If adding a new mode, set this to the last good mode number
...
struct GPRMC_Data {                   //$GPRMC,161724.00,A,3857.22509,N,00859.99477,W,0.207,,311025,,,A*61
  ...
  char ns;          // N/S
  char lon[13];     // dddmm.mmmm
  char ew;          // E/W
  char speed[8];    // knots
  char date[7];     // ddmmyy
  bool valid;       // true se status == 'A'
  uint32_t  llat;
  uint32_t  llon;
  uint32_t  kmh;
};
...
#define DEG_SCALE 100000UL  // resultado final = graus × 100000

// lat: "DDMM.MMMMM"  (ex: "3857.22509")
// lon: "DDDMM.MMMMM" (ex: "00859.99477")



uint32_t parseLat(const char *lat)
{
    // graus: DD
    uint32_t graus = (uint32_t)((lat[0]-'0')*10 + (lat[1]-'0'));

    // minutos: MM
    uint32_t minutos = (uint32_t)((lat[2]-'0')*10 + (lat[3]-'0'));

    // fração de minutos: .MMMMM
    uint32_t fracMin = 
        ((uint32_t)(lat[5]-'0') * 10000U) +
        ((uint32_t)(lat[6]-'0') * 1000U) +
        ((uint32_t)(lat[7]-'0') * 100U) +
        ((uint32_t)(lat[8]-'0') * 10U) +
        ((uint32_t)(lat[9]-'0'));

    uint32_t totalMinX100000 = minutos * 100000UL + fracMin;
    uint32_t fracDeg = totalMinX100000 / 60UL;

    return graus * DEG_SCALE + fracDeg;
}

uint32_t parseLon(const char *lon)
{
    // graus: DDD
    uint32_t graus = (uint32_t)((lon[0]-'0')*100 + (lon[1]-'0')*10 + (lon[2]-'0'));

    // minutos: MM
    uint32_t minutos = (uint32_t)((lon[3]-'0')*10 + (lon[4]-'0'));

    // fração de minutos: .MMMMM
    uint32_t fracMin = 
        ((uint32_t)(lon[6]-'0') * 10000U) +
        ((uint32_t)(lon[7]-'0') * 1000U) +
        ((uint32_t)(lon[8]-'0') * 100U) +
        ((uint32_t)(lon[9]-'0') * 10U) +
        ((uint32_t)(lon[10]-'0'));
...
uint16_t parse_knots(const char *s)
{
    uint16_t val = 0;
    uint8_t fracCount = 0;
    while (*s) {
        if (*s >= '0' && *s <= '9') {
            val = val * 10 + (*s - '0');
            if (fracCount > 0) fracCount++;
        } else if (*s == '.') {
            fracCount = 1;
        }
        s++;
    }
    while (fracCount++ < 4)  // normaliza para 3 casas decimais
        val *= 10;
    return val; // knots * 1000
}

bool parseGPRMC_bytewise(GPRMC_Data *gps, char c) {
  ...
  switch (state) {
    ...
    case 1: state = (c == 'G') ? 2 : 0; break;
    case 2: state = (c == 'P') ? 3 : 0; break;
    case 3: state = (c == 'R') ? 4 : 0; break;
    case 4: state = (c == 'M') ? 5 : 0; break;
    case 5: state = (c == 'C') ? 6 : 0; break;
    case 6: if (c == ',') { field = 0; idx = 0; state = 7; } else {state = 0;} break;

    case 7:
      if (c == ',' || c == '\n') {
        // fim do campo
        if (field == 0) gps->time[idx] = '\0';
        else if (field == 1) gps->status = gps->status ? gps->status : 'V';
        else if (field == 2) gps->lat[idx] = '\0';
        else if (field == 3) ; // N/S já lido
        else if (field == 4) gps->lon[idx] = '\0';
        else if (field == 5) ; // E/W já lido
        else if (field == 6) gps->speed[idx] = '\0';
        else if (field == 8) gps->date[idx] = '\0';
        ...
        if (c == ',') { field++; idx = 0; }
          ...
          if (gps->valid)
            {                                                                     //$GPRMC,161724.00,A,3857.22509,N,00859.99477,W,0.207,,311025,,,A*61
              ...
              gps->date[4]= c;
              c= gps->date[1];
              gps->date[1]= gps->date[5];
              gps->date[5]= c;
              gps->time[7]= gps->time[5]; // change time format from 161724 to 16:17:24
              gps->time[6]= gps->time[4];
              gps->time[4]= gps->time[3];
              gps->time[3]= gps->time[2];
              gps->time[2]= gps->time[5]= ':';
              gps->time[8]= 0;
//              NewSerial.println(gps->lon);
              gps->llon= parseLon(gps->lon);
//              NewSerial.println(gps->lat);
              gps->llat= parseLat(gps->lat);
             //gps->flat= (gps->lat[0] -'0')*10 + gps->lat[1] -'0' + ((gps->lat[2] -'0') * 10 + gps->lat[3] -'0' + (gps->lat[5] -'0') * 0.1 + (gps->lat[6] -'0') * 0.01 + (gps->lat[7] -'0') * 0.001 + (gps->lat[7] -'0') * 0.0001) / 60;
              //gps->flon= (gps->lon[0] -'0')*100 + (gps->lon[1] -'0') * 10 + gps->lon[2] -'0' + ((gps->lon[3] -'0') *10 + gps->lon[4] -'0' + (gps->lon[6] -'0') * 0.1 + (gps->lon[7] -'0') * 0.01 + (gps->lon[8] -'0') * 0.001 + (gps->lon[9] -'0') * 0.0001) / 60;
              gps->kmh= parse_knots (gps->speed);
//              NewSerial.println(gps->kmh);
              gps->kmh= gps->kmh * 1852UL / 1000UL;
            }
          return gps->valid; //true; // trama completa
        }
      } else {
        // acumular dados por campo
        if (field == 0 && idx < sizeof(gps->time)-1) gps->time[idx++] = c;
        else if (field == 1 && idx == 0) gps->status = c;
        else if (field == 2 && idx < sizeof(gps->lat)-1) gps->lat[idx++] = c;
        else if (field == 3 && idx == 0) gps->ns = c;
        else if (field == 4 && idx < sizeof(gps->lon)-1) gps->lon[idx++] = c;
        else if (field == 5 && idx == 0) gps->ew = c;
        else if (field == 6 && idx < sizeof(gps->speed)-1) gps->speed[idx++] = c;
        else if (field == 8 && idx < sizeof(gps->date)-1) gps->date[idx++] = c;
      }
      break;
  }

  return false; // ainda a receber
}
...
void loop(void)
{
  ...
  if (setting_systemMode == MODE_NEWLOG || setting_systemMode == MODE_ROTATE || setting_systemMode == MODE_GPS)
  {
...
char* newLog(void)
{
  ...
  if (setting_systemMode == MODE_GPS)
    {
      NewSerial.println(F("3"));
      newFileName[0]= 'd';
      newFileName[1]= 'u';
      newFileName[2]= 'm';
      newFileName[3]= 'm';
...
byte appendFile(char* fileName)
{
              ...
              {                               //$GPRMC,,V,,,,,,,,,,N*53
                if (decdaydigit != gps.date[4] || uniddaydigit != gps.date[5])
                  {             // day change, close current file and open another
                    decdaydigit= gps.date[4];  
                    uniddaydigit= gps.date[5];
                    workingFile.sync();
                    workingFile.close(); // Done recording, close out the file
                    digitalWrite(stat1, LOW); // Turn off indicator LED
                    NewSerial.print(F("~")); // Indicate a successful record

                    fileName[0]= gps.date[0];
                    fileName[1]= gps.date[1];
                    fileName[2]= gps.date[2];
                    fileName[3]= gps.date[3];
                    fileName[4]= gps.date[4];
                    fileName[5]= gps.date[5];
                    fileName[6]= '.';
                    fileName[7]= 't';
                    fileName[8]= 'x';
                    fileName[9]= 't';
  ...
  if (setting_max_escape_character > 0 && setting_systemMode != MODE_GPS)
...
//    setting_systemMode = MODE_NEWLOG; //By default, unit will turn on and go to new file logging
    setting_systemMode = MODE_GPS; //By default, unit will turn on and go to new file logging
...
//    setting_escape_character = 26; //Reset escape character to ctrl+z
    setting_escape_character = 36; //Reset escape character to $
...
void systemMenu(void)
{
  ...
  while (1)
  {
    ...
    NewSerial.println(F("g) Mode GPS"));
    ...
    if (command == 'g')
    {
      ...
      EEPROM.write(LOCATION_SYSTEM_SETTING, MODE_GPS);
      recordConfigFile(); //Put this new setting into the config file
      return;
    }
    if (command == '1')
...
{
    // graus (DD)
NewSerial.println(lat);
    uint32_t graus = (lat[0]-'0')*10 + (lat[1]-'0');
    // minutos (MM)
    uint32_t minutos = (lat[2]-'0')*10 + (lat[3]-'0');
    // fracMin = 5 dígitos após o ponto (MMMMM)
    uint32_t fracMin = (lat[5]-'0')*10000 + (lat[6]-'0')*1000 +
                       (lat[7]-'0')*100 + (lat[8]-'0')*10 + (lat[9]-'0');
NewSerial.println(fracMin);
    // totalMinX100000 = minutos * 100000 + fracMin
    uint32_t totalMinX100000 = minutos * 100000UL + fracMin; // <= 5_999_999
NewSerial.println(totalMinX100000);
    // fracDeg (em graus * 100000) = totalMinX100000 / 60
    uint32_t fracDeg = totalMinX100000 / 60UL;
NewSerial.println(fracDeg);
NewSerial.println();
    return graus * DEG_SCALE + fracDeg;
}
...
uint32_t parseLon(const char *lon)
{
NewSerial.println(lon);
    // graus (DDD)
    uint32_t graus = (lon[0]-'0')*100 + (lon[1]-'0')*10 + (lon[2]-'0');
    // minutos (MM)
    uint32_t minutos = (lon[3]-'0')*10 + (lon[4]-'0');
    // fracMin = 5 dígitos após o ponto (MMMMM)
//    uint32_t fracMin = (lon[6]-'0')*10000 + (lon[7]-'0')*1000 + (lon[8]-'0')*100 + (lon[9]-'0')*10 + (lon[10]-'0');
    uint32_t fracMin = (lon[6]-'0')*10000l;
NewSerial.println(fracMin);
    fracMin+= (lon[7]-'0')*1000;
NewSerial.println(fracMin);
    fracMin+= (lon[8]-'0')*100;
NewSerial.println(fracMin);
    fracMin+= (lon[9]-'0')*10;
